...
/* On mention caas example
ex User enters @botname 'caas' phrase, the bot will post stuff
*/
framework.hears('caas', function (bot, trigger) {
  console.log("someone asked for CaaS Stuff.");
  responded = true;
  bot.say('Hello there, General Kenobi! I am your CaaS support buddy Count AsAvese! \n' +
  'Prepare to meet your maker \n');
  console.log("someone asked for a caas card");
  let avatar = trigger.person.avatar;

  cardJSON.body[0].columns[0].items[0].url = (avatar) ? avatar : `${config.webhookUrl}/missing-avatar.jpg`;
  cardJSON.body[0].columns[0].items[1].text = 'This is the caas card. Congratulations!';
  bot.sendCard(caasJSON, 'This is customizable fallback text for clients that do not support buttons & cards');
});  
